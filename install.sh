...
if ! [[ "$USER_REGION" =~ ^[a-z]{2}-[A-Z]{2}$ ]]; then
  echo -e "${YELLOW}Invalid region format; using default ${DEFAULT_REGION}.${NC}"
  USER_REGION="$DEFAULT_REGION"
fi
...
if echo "$EXISTING_CRON" | grep -q "$INSTALL_DIR/bing_wallpaper.sh"; then
  ...
  (
    echo "$EXISTING_CRON" | grep -Fv -- "$INSTALL_DIR/bing_wallpaper.sh"
    echo "$CRON_JOB"
  ) | crontab -
else
