# debian:trixie-slim, pinned by digest only
...
# One RUN keeps the image to a single tooling layer:
# 1. Bootstrap ca-certificates from the image's default sources so the pinned
#    Debian snapshot can then be used over HTTPS.
# 2. Install pinned packages from the snapshot:
#    bats (tests), curl (used by script), procps (pgrep), kcov (coverage), linters.
# 3. Install Python/Node linters from prebuilt artifacts only (no install scripts).
# 4. Install checksummed hadolint, shellcheck, and shfmt release binaries.
RUN apt-get update \
    ...
    && snapshot_main="https://snapshot.debian.org/archive/debian/${DEBIAN_SNAPSHOT}" \
    && snapshot_security="https://snapshot.debian.org/archive/debian-security/${DEBIAN_SNAPSHOT}" \
    ...
    && case "$arch" in \
        amd64) bin_arch="x86_64"; hadolint_arch="x86_64"; \
        ...
        arm64) bin_arch="aarch64"; hadolint_arch="arm64"; \
    ...
    && curl --proto '=https' --tlsv1.2 -fsSL "${hadolint_url}/hadolint-linux-${hadolint_arch}" \
        -o /usr/local/bin/hadolint \
    && printf '%s  %s\n' "$hadolint_sha256" /usr/local/bin/hadolint | sha256sum -c - \
    && chmod +x /usr/local/bin/hadolint \
    && sc_url="https://github.com/koalaman/shellcheck/releases/download/${SHELLCHECK_VERSION}" \
    && curl --proto '=https' --tlsv1.2 -fsSL "${sc_url}/shellcheck-${SHELLCHECK_VERSION}.linux.${bin_arch}.tar.xz" \
    ...
    && useradd --create-home --uid 1001 --user-group --shell /bin/bash tester \
    && install -d -o 1001 -g 1001 /app
...
# Tests and linters run unprivileged as uid 1001, which matches the GitHub-hosted runner
# user, so bind-mounted checkouts and coverage directories stay writable without chmod.
# Linter caches go to /tmp so a read-only or foreign-owned mount never blocks them.
